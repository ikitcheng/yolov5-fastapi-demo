...
import random
import config
import waste_analysis as waste
...
async def detect_via_web_form(
    ...
    img_batch = [
        cv2.imdecode(np.fromstring(await file.read(), np.uint8), cv2.IMREAD_COLOR)
    ...
    return templates.TemplateResponse(
        ...
        {
            "request": request,
            "bbox_image_data_zipped": zip(
                img_str_list, json_results
